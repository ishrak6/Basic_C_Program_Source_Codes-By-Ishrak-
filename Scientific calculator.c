#include<string.h> #include<mouse.h> #include<stdlib.h> #include<conio.h> #include<math.h> #include<graphics.h> #include<stdio.h> #include<alloc.h>
#define pi 3.1415265 union REGS i,o;
int sr,dpf;
...
void draw()	//-----DRAW THE BODY OF CALCULATOR
{
    ...
    initgraph(&gd,&gm,"c:\\tc\\bgi");	//INITIALISE GRAPHICS z2=(char * ) malloc(sizeof(char)*10);		/*ALLOCATE MEMORY TO z2*/ x=getmaxx();y=getmaxy();
    setcolor(DARKGRAY);
    setfillstyle(1,CYAN);	//CYAN outer body floodfill(5,5,DARKGRAY);

    setcolor(BLACK);
    setfillstyle(1,LIGHTGRAY);
    rectangle(20,20,x-20,y-20);	//grey calculator body floodfill(50,50,BLACK);

    setcolor(BLACK);	//black outline line(20,20,x-20,20);
    line(20,20,20,y-20);
    ...
    rectangle(30,55,x-30,70);	//---white i/o box floodfill(32,59,WHITE);
    ...
    for(x1; x1<501; x1+=30)
    {
        ...
        xy1[i]=x1;	/*store X coordinates*/ i++;
        x1+=70;
    }
    /*-----------------------------------------------------------------*/
                    ...
                    if(x>=x1 && y>=y1 && x<=x1+40 && y<=y1+40)
                    {
                        ...
                        if((strcmp(no[i+j],no[10]))==0)
                        {
                            if(dpf==0)
                            {
                                ...
                                strcat(s,no[i+j]);	//save no. to string s dpf=1;
                            }
                        }
                        else
                        ...
                        s
NEXT:
                        n=_atold(s);	//array to long double setcolor(DARKGRAY);
...
long double alg(long double n, int y )
{
    ...
    if(y>100 && y<140)
    {
        box(180,100,40,BLACK,WHITE);	//--add
        delay(200);
...
long com(long b,int k)
{
    char *str;
    int i=0;
    itoa(b,str,k);	// integer to array if(k==16)
    {
        while(str[i]!='\0')
        {
            if(str[i]>96 && str[i]<103) str[i]-=32;
            i++;
        }
    }
    setcolor(DARKGRAY);
...
password()
{
    ...
    pass[i]='\0';
    if(!strcmp(pass,"as"))	//compare entered string with password return 0;
        else cleardevice();
    rectangle(15,15,615,465);
    setcolor(LIGHTBLUE);
    setfillstyle(1,LIGHTBLUE);
...
//Changing the blue rectangle contents press()
{
    int x;
    x=getmaxx();
    setcolor(BLACK);
    setfillstyle(1,RED);
    bar(21,20,x-21,40);
    floodfill(25,25,BLACK);	/* blue rectangle*/ setcolor(WHITE);
    outtextxy(x-250,27,"Press Any Key To Continue");	/*---Continue---*/ getch();
